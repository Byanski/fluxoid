...
# 1. curl (needed before everything else for nix install)
...
if ! command -v nix &>/dev/null; then
    info "Nix not found — installing (this may take a few minutes)..."
    curl --proto '=https' --tlsv1.2 -sSf -L https://install.determinate.systems/nix | sh -s -- install --no-confirm
    # The Determinate installer writes this file
    if [ -f /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh ]; then
        . /nix/var/nix/profiles/default/etc/profile.d/nix-daemon.sh
    fi
    if ! command -v nix &>/dev/null; then
        error "Nix was installed but could not be loaded into this session.\nPlease open a new terminal and run this script again."
    fi
...
if ! command -v devenv &>/dev/null; then
    info "devenv not found — installing..."
    nix profile install --accept-flake-config github:cachix/devenv/latest
    if ! command -v devenv &>/dev/null; then
        error "devenv was installed but could not be loaded into this session.\nPlease open a new terminal and run this script again."
    fi
    success "devenv installed: $(devenv --version 2>/dev/null || echo 'version unknown')"
else
